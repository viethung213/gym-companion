...
| **FR-AC-08** | **Tập thêm bài bổ trợ linh hoạt (Extra Workouts)**: Sau khi hoàn thành buổi tập chính (bao gồm Cool-down của FR-AC-07), ứng dụng cung cấp tùy chọn "Tập thêm bài bổ trợ". Backend Go tự động loại bỏ 100% bài đã tập, loại bỏ bài compound nặng và trả về 3 nhóm an toàn qua API `POST /api/v1/users/{user_id}/daily-workout-plans/today/extra-exercises`: (a) Accessory/Core, (b) Cardio LISS, (c) Deep Mobility (phản hồi < 100ms, API Go thuần không qua AI). | S |
| **FR-AC-09** | **Phản hồi & Điều chỉnh mức độ của Người dùng (User Exercise Feedback & Intensity Control)**: Cho phép người dùng chủ động gửi phản hồi bài tập: (a) *Phản hồi "Bài quá dễ"*: Kích hoạt luồng Fast-Track (`BR-AC-02`) tự động tăng 15-30% mức tạ hoặc volume/reps ở các buổi tiếp theo, (b) *Phản hồi "Chán bài tập"*: Ghi nhận log `exercise_feedback: BORING` vào Prompt Context để AI Agent tự nhận biết, chủ động thay thế bài tập tương đương cùng nhóm cơ cho buổi sau và tạo lời giải thích cá nhân hóa. | M |
...
| **BR-AC-07** | Tập luyện | **Signal B3 (Tập quá tải - Overtraining)**: Kích hoạt khi user tập $\ge 2$ buổi/ngày hoặc RPE trung bình $\ge 8.5$ liên tục $\ge 5$ buổi → Mặc định phát cảnh báo và đổi giáo án buổi 2 sang Active Recovery / Cardio LISS / Mobility. Nếu User chọn Override "Vẫn muốn tập nặng", Go Engine ép buộc chọn nhóm cơ khác hoàn toàn với buổi 1 (khóa sạch nhóm cơ vừa tập buổi sáng). |
