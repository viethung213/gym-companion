...
| 1.3.0 | 30/06/2026 | Tinh giản danh sách tác nhân (Actors) và chuẩn hóa luồng chức năng, chuyển đổi các AI Engine thành module nội bộ của hệ thống. | Antigravity AI |
| 1.4.0 | 30/06/2026 | Thay đổi luồng nghiệp vụ Khởi tạo lịch tập sang Mô hình Hybrid (Macro 4 tuần - Micro 1 tuần - Daily Bài tập). | Antigravity AI |
...
|---|---|---|---|
| **ACT-01** | End User (Người tập) | Người sử dụng ứng dụng để thiết lập hồ sơ sức khỏe, lên lịch tập, thực hiện bài tập dưới sự giám sát của AI Camera, theo dõi dinh dưỡng và xem báo cáo tiến trình. | High |
| **ACT-02** | System Administrator (Quản trị viên) | Quản trị viên hệ thống: Quản lý thư viện bài tập, cơ sở dữ liệu món ăn, kiểm tra dữ liệu, báo cáo hoạt động và bảo mật. | Medium |

*Lưu ý: Các thành phần xử lý thông minh như AI Coach, AI Camera, và AI Nutrition được định nghĩa là các module nghiệp vụ nội bộ của hệ thống FITAI, không phải là tác nhân bên ngoài.*
...
```
[Người dùng] ──► Nhập Thông tin Cá nhân & Chỉ số Cơ thể (Tuổi, Giới tính, Cao, Nặng)
     │
     ▼
[Người dùng] ──► Chọn Trình độ (Mới tập/Có kinh nghiệm) & Mục tiêu (Tăng cơ/Giảm mỡ)
     │
     ▼
[Người dùng] ──► Khai báo Hạn chế Sức khỏe (Chấn thương cũ, Bệnh lý mãn tính)
     │
     ▼
[Hệ thống (AI Coach)] ──► Tính toán User Fitness Score, TDEE & Lộ trình 4 tuần (Macrocycle)
     │
     ▼
[Hệ thống (AI Coach)] ──► Sinh Lịch các buổi tập Tuần 1 & Chi tiết Bài tập Ngày 1
     │
     ▼
[Hệ thống (AI Nutrition)] ──► Đề xuất thực đơn dinh dưỡng cá nhân hóa
     │
     ▼
[Người dùng] ──► Chọn Khung giờ nhắc nhở cố định & Xác nhận kích hoạt
```

### 3.2 Quy trình Luyện tập dưới AI Camera (Workout Execution)

```
[Người dùng] ──► Mở ứng dụng vào ngày có lịch tập
     │
     ▼
[Hệ thống (AI Coach)] ──► Sinh/Cập nhật chi tiết Bài tập hôm nay (dựa trên thể trạng/DOMS)
     │
     ▼
...
[Người dùng] ──► Bật Camera & Điều chỉnh khoảng cách/Ánh sáng theo chỉ dẫn
     │
     ▼
[Hệ thống (AI Camera)] ──► Tracking skeleton (Khung xương) & Ước lượng cân nặng tạ thực tế
     │
     ▼
[Hệ thống (AI Camera)] ──► Đếm số rep, đo lường % hoàn thiện rep (ROM %)
...
| **FR-UM-04** | Nhắc nhở lịch tập tự động | Hệ thống tự động quét và gửi Push Notification trước khung giờ tập cố định 15 phút, nội dung thông báo được cá nhân hóa theo phong cách AI Coach đã thiết lập. | Hệ thống | S |
...
| Mã yêu cầu | Tên yêu cầu | Mô tả nghiệp vụ chi tiết | Tác nhân | Độ ưu tiên |
|---|---|---|---|---|
| **FR-AC-01** | Khởi tạo Lộ trình & Lịch tập tuần | Hệ thống phân tích chỉ số cơ thể, mục tiêu để thiết lập cấu trúc Lộ trình 4 tuần (Split, Tần suất) và sinh lịch tập (ngày tập) cho tuần đầu tiên. | Hệ thống | M |
| **FR-AC-02** | Sinh Bài tập Hàng ngày | Hệ thống tự động sinh chi tiết bài tập cụ thể, số set, rep, và cân nặng cho buổi tập ngày hôm đó dựa trên phong độ buổi trước, RPE, và độ mỏi cơ (DOMS). | Hệ thống | M |
| **FR-AC-03** | Điều chỉnh lập tức khi chấn thương/Thiếu dụng cụ | Hệ thống sẽ thay thế ngay lập tức các bài tập tương đương nếu người dùng báo chấn thương đột xuất hoặc thiếu thiết bị tại phòng tập. | Hệ thống, ACT-01 | S |
| **FR-AC-04** | Tự động chèn tuần giảm tải (Deload) | Hệ thống tự động theo dõi mức độ mệt mỏi tích lũy (Cumulative Fatigue) và chèn tuần giảm tải khi người dùng chững lại. Hệ thống gửi tin nhắn động viên cá nhân hóa. | Hệ thống | S |
...
| Mã yêu cầu | Tên yêu cầu | Mô tả nghiệp vụ chi tiết | Tác nhân | Độ ưu tiên |
|---|---|---|---|---|
| **FR-CC-01** | Tracking Khung xương | Hệ thống phân tích luồng video camera trước/sau để xác định 33 điểm khớp chính trên cơ thể người tập. | Hệ thống | M |
| **FR-CC-02** | Đo lường góc ROM % | Hệ thống tính toán biên độ chuyển động (ROM) của các khớp và đưa ra tỷ lệ % hoàn thiện của mỗi lần lặp (rep). | Hệ thống | M |
| **FR-CC-03** | Phát hiện lỗi tư thế | Hệ thống tự động so sánh tọa độ các khớp với mô hình chuyển động chuẩn để phát hiện lỗi kỹ thuật (VD: võng lưng, gối chụm...). | Hệ thống | M |
| **FR-CC-04** | Cảnh báo thời gian thực | Hệ thống hiển thị visual overlay và phát tín hiệu giọng nói hướng dẫn sửa lỗi tức thì với độ trễ tối đa < 500ms. | Hệ thống | M |
| **FR-CC-05** | Chấm điểm kỹ thuật | Hệ thống tính điểm kỹ thuật (Form Score từ 0-100) cho từng rep dựa trên sự kết hợp giữa ROM %, căn chỉnh khớp và tốc độ thực hiện. | Hệ thống | S |
...
| Mã yêu cầu | Tên yêu cầu | Mô tả nghiệp vụ chi tiết | Tác nhân | Độ ưu tiên |
|---|---|---|---|---|
| **FR-WL-01** | Ghi chép buổi tập tự động | Hệ thống tự động điền số rep đã đếm được và % hoàn thiện. Hệ thống ước lượng cân nặng tạ thực tế thông qua việc nhận dạng đĩa tạ và tốc độ nâng của người dùng để tránh nhập khống dữ liệu. | Hệ thống | M |
| **FR-WL-02** | Ghi chép thủ công | Cho phép người dùng chỉnh sửa số rep và cân nặng thực tế nếu hệ thống ước lượng hoặc đếm chưa chính xác trước khi bấm lưu set. | ACT-01 | M |
| **FR-WL-03** | Tương tác âm thanh thông minh | Hệ thống cho phép người dùng chọn nhạc nền buổi tập (EDM/Lo-fi). Hệ thống tự giảm âm lượng nhạc nền (Audio Ducking) khi phát âm thanh sửa lỗi kỹ thuật. | ACT-01, Hệ thống | S |
| **FR-WL-04** | Ghi nhận Kỷ lục Cá nhân (PR) | Hệ thống tự động tính toán 1RM ước tính (Epley Formula) sau mỗi buổi tập và vinh danh bằng ăn mừng nếu đạt PR mới. | Hệ thống | S |
...
| Mã yêu cầu | Tên yêu cầu | Mô tả nghiệp vụ chi tiết | Tác nhân | Độ ưu tiên |
|---|---|---|---|---|
| **FR-NU-01** | Tính toán kcal cá nhân hóa | Hệ thống tính toán TDEE và macronutrients hàng ngày riêng biệt cho từng cá nhân, dựa theo công thức Mifflin-St Join và khối lượng vận động thực tế. | Hệ thống | M |
| **FR-NU-02** | Đa lựa chọn theo ngân sách | Mỗi bữa ăn (Sáng, Trưa, Tối) gợi ý tối thiểu 3 lựa chọn món ăn đạt chuẩn macro, chia theo mức giá: Tiết kiệm, Phổ thông, và Thoải mái. | Hệ thống | S |
| **FR-NU-03** | Thuật toán Anti-Repetition | Hệ thống loại trừ các nguồn protein chính đã dùng trong 7 ngày, tinh bột trong 5 ngày và chủ đề món trong 3 ngày để chống lặp lại món ăn gây ngán. | Hệ thống | M |
...
| **FR-PT-02** | Phân tích xu hướng (Analytics) | Hệ thống vẽ biểu đồ xu hướng biến động cân nặng (moving average), xu hướng tăng sức mạnh (1RM) và biểu đồ điểm kỹ thuật trung bình. | Hệ thống | S |
| **FR-PT-03** | AI phân tích chuyên sâu | Hệ thống định kỳ gửi báo cáo phân tích chuyên sâu về tiến độ đạt mục tiêu của người dùng kèm lời khuyên tối ưu. | Hệ thống | S |
...
| **BR-AC-01** | Tập luyện | Khi sinh lịch tập tuần (Microcycle), AI Coach không được phép xếp lịch vượt quá **6 buổi/tuần** và bắt buộc phải có tối thiểu **1 ngày nghỉ hoàn toàn**. |
| **BR-AC-02** | Tiến độ | Khi sinh bài tập hàng ngày, tốc độ tăng tiến khối lượng tập luyện (Progressive Overload) do AI đề xuất không được vượt quá **10% tổng volume** so với buổi tập tương tự gần nhất. |
| **BR-AC-03** | RPE Feedback | Nếu người dùng ghi nhận RPE $\ge 9$ (quá mệt) hoặc $\le 5$ (quá nhẹ), hệ thống phải tự động điều chỉnh mức tạ gợi ý từ 5-10% cho chuyển động đó ở buổi tập tiếp theo. |
...
*Tài liệu Đặc tả Yêu cầu Nghiệp vụ Cốt lõi theo chuẩn BABOK v3.0 – Phê duyệt ngày 30/06/2026*
