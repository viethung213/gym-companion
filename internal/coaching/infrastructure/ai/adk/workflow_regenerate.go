...
func (c *CoachingContextAgent) buildRegeneratePendingAgent(_ context.Context) (agent.Agent, error) {
	node := workflow.NewDynamicNode(
		...
		func(nodeCtx agent.Context, userID string, _ func(*session.Event) error) (*PlanResult, error) {
			...
			coachInput.Flow = FlowRegenerate
	...
	wf, err := workflow.New("regenerate_wf", workflow.Chain(workflow.Start, node))
	if err != nil {
