...
func TestCreateAdhocSessionHandler_Success(t *testing.T) {
	...
	rm, err := roadmap.InitiateRoadmap(info, now)
	if err != nil {
		t.Fatalf("InitiateRoadmap: %v", err)
	}

	// Find or create a day plan for today
	weeks := rm.Weeks()
	if len(weeks) == 0 {
		t.Fatal("no weeks created")
	}

	dayPlan := findOrCreateDayPlan(t, weeks[0], today)
