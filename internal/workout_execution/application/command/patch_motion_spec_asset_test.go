...
func TestPatchMotionSpecificationAssetHandler(t *testing.T) {
	...
	t.Run("Patch dialogue config creates file on S3 and updates spec", func(t *testing.T) {
		draft := aggregate.NewDraftMotionSpecification("ex-squat", "Squat", "", "")
	...
	t.Run("Partial patch updates existing keys and deletes specified key", func(t *testing.T) {
		...
		existingSpec := aggregate.RestoreMotionSpecification("ex-squat", "Squat", "http://cdn/detector.onnx", "http://cdn/skeleton.onnx", "http://cdn/rules/ex-squat_rule.json", "http://cdn/dialogue/ex-squat_dialogue.json", "side", true, time.Now().UTC(), time.Now().UTC())
