...
description: 'Intelligent branch creator that analyzes git status/diff and creates appropriate branches branching from and merging to the main branch.'
---
...
```xml
<instructions>
	<title>Git Branch Creator</title>
	<description>This prompt analyzes your current git changes using git status and git diff (or git diff --cached), then intelligently determines the appropriate branch type and creates a semantic branch name.</description>
	<note>
		Just run this prompt and Copilot will analyze your changes and create the appropriate branch for you.
	</note>
</instructions>
...
3. Analyze the changes using the Git Branch Analysis Framework below.
4. Determine the appropriate branch type based on the analysis.
5. Generate a semantic branch name following standard branching conventions.
6. Fetch the latest changes from the remote repository, then create the new branch based on the updated `main` branch.
7. Provide a summary of the analysis and next steps.

### Git Branch Analysis Framework
...
<analysis-process>
	...
	<step-2>
		<title>Git Branch Classification</title>
		<description>Map the changes to appropriate branch type</description>
		<decision-tree>
...
<validation>
	<pre-analysis>
		...
		<check>Current branch is appropriate starting point (main for features/releases/hotfixes)</check>
		<check>Fetch the latest changes from the remote repository to ensure the start point is up to date</check>
	</pre-analysis>

	<analysis-quality>
		<check>Change analysis covers all modified files</check>
		<check>Branch type selection follows standard branching principles</check>
	...
	<execution-safety>
		...
		<check>Ensure the checkout command explicitly specifies 'main' as the starting point</check>
...
<execution-protocol>
	...
	<branch-creation>
		<command>git fetch origin main && git checkout -b [branch-name] origin/main --no-track</command>
	...
	<fallback-options>
		<offline-fallback>If offline or remote fetch fails, fallback to creating from local main: git checkout -b [branch-name] main</offline-fallback>
...
<git-branching-reference>
	<main-branches>
		<main>Production-ready code and development integration branch</main>
	</main-branches>

	<supporting-branches>
		<feature>Branch from main, merge back to main</feature>
		<release>Branch from main, merge to main</release>
		<hotfix>Branch from main, merge to main</hotfix>
	</supporting-branches>
